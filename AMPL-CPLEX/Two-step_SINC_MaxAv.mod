...
set F := 0..comb;
#Combinations of double link failures of virtual network
...
#maximize AvailMapping: (num_top*(comb+1)) - (sum{k in F, v in V} am[v,k]);
maximize AvailMapping: 100*( (num_top*(comb+1)) - (sum{k in F, v in V} am[v,k]) ) - 0.1*(sum{(i,j) in A, (v,s,t) in Btot} q[i,j,v,s,t]);
...
# SVNM problem
problem SVNM1: qa,q,g,r,am, AvailMapping, flow,flow1,flow2,flow3,flow4,flow5,flow6,bidirectionality,bidirectionality2, survivability, downVirLinks,downVirLinks2,cutsetsDown,cutsetsDown2,virtNetDownMap,virtNetDownMap2;
