...
def analyze_availability(topology, av_matrix, twc_matrix, shared_link_matrix):
    x_tick_label_list = ['5', '10', '15', '20', '25']
    scenario_legend_list = ["SP", "SVNM", "SINC", "SINC+"]
    file_name = "description/large_instance_increasing_virtual_networks_" + topology + "_av.txt"
    with open(file_name, "w") as f:
        # the maximum availability of SP is xx%
        max_av_sp = np.max(av_matrix[0])
        f.write("The maximum availability of SP is " + str(max_av_sp) + "%\n")
        # the maximum availability of SVNM is xx%
        max_av_svnm = np.max(av_matrix[1])
        f.write("The maximum availability of SVNM is " + str(max_av_svnm) + "%\n")

        # the availability of \textit{SINC} increases from around xx\% to xx\% when the number of VNs increases from 5 to 25
        av_sinc_5 = av_matrix[2][0]
        av_sinc_25 = av_matrix[2][-1]
        f.write("The availability of SINC increases from around " + str(av_sinc_5) + "% to " + str(av_sinc_25) + "% when the number of VNs increases from 5 to 25\n")
        # availability of SINC is around up to xx\% higher than that \textit{SVNM} for VN requests from 5 to 25
        current_difference = av_matrix[2] - av_matrix[1]
        max_difference = np.max(current_difference)
        f.write("The availability of SINC is around up to " + str(max_difference) + "% higher than that of SVNM for VN requests from 5 to 25\n")
        # availability of SINC+ is around up to xx\% higher than that \textit{SINC} for VN requests from 5 to 25
        current_difference = av_matrix[3] - av_matrix[2]
        max_difference = np.max(current_difference)
        f.write("The availability of SINC+ is around up to " + str(max_difference) + "% higher than that of SINC for VN requests from 5 to 25\n")

        f.write("\n")
        current_difference_twc = twc_matrix[3] - twc_matrix[2]
        relative_difference_twc = 100 * current_difference_twc / twc_matrix[2]
        max_relative_difference_twc = np.max(relative_difference_twc)
        # the additional TWC required by \textit{SINC+} compared to \textit{SINC} reduces from xx\% to xx\% when increasing the number of VNs
        f.write("The additional TWC required by SINC+ compared to SINC reduces from " + str(relative_difference_twc[0]) + "% to " + str(relative_difference_twc[-1]) + "% when increasing the number of VNs\n")

        f.write("\n")
        # dditional VLs reconnected with sharing in SINC+ compared to SINC reduces from xx\% to xx\% when the number of VNs increases from 5 to 25
        current_difference_shared_link = shared_link_matrix[3] - shared_link_matrix[2]
        relative_difference_shared_link = 100 * current_difference_shared_link / shared_link_matrix[2]
        max_relative_difference_shared_link = np.max(relative_difference_shared_link)
        f.write("The additional VLs reconnected with sharing in SINC+ compared to SINC reduces from " + str(relative_difference_shared_link[0]) + "% to " + str(relative_difference_shared_link[-1]) + "% when the number of VNs increases from 5 to 25\n")

        # the maximum additional VLs reconnected with sharing in all cases is xx
        max_shared_link = np.max(shared_link_matrix)
        f.write("The maximum additional VLs reconnected with sharing in all cases is " + str(max_shared_link) + "\n")
