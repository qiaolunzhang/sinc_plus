...
def get_columns_max_list(topology, num_of_vn, num_of_vl_list, num_instance,
                           scenario_list: list=None, scenario_legend_list: list=None, column_index_list: list = None):
    num_scenarios = len(scenario_list)
    if scenario_list is None:
        scenario_list = ["scenario-1", "scenario-2", "scenario-3", "scenario-4", "scenario-7"]
    if scenario_legend_list is None:
...
def get_matrix_columns(topology, num_of_vn, num_of_vl_list, num_instance,
                           scenario_list: list=None, scenario_legend_list: list=None, column_index_list: list = None):
    num_scenarios = len(scenario_list)
    if scenario_list is None:
        scenario_list = ["scenario-1", "scenario-2", "scenario-3", "scenario-4", "scenario-7"]
...
def plot_av_twc_computation_time_sp(topology, av_matrix, twc_matrix, shared_link_matrix, double_axis_flag=False):
    ...
    x_tick_label_list = ['0.5', '0.6', '0.7', '0.8', '0.9', '1']
    ...
    ax[0].set_xlabel('Connectivity degree\n (a) Availability')
    if double_axis_flag:
    ...
    if not double_axis_flag:
        ...
        axins = inset_axes(ax[0], width="100%", height="100%",
                           ...
                           # loc=1,
                           loc='upper left',
                           bbox_to_anchor=(0.11, 1 - 0.3, .05, .25), bbox_transform=ax[0].transAxes,
        ...
        selected_data = [[av_matrix[2, 0]], [av_matrix[3, 0]]]
        ...
        axins.set_xticklabels(['5'])  # Labels specifically for the zoomed columns
    ...
    # ax[1].set_ylim(400, 1900)
    ax[1].set_ylim(400, 1200)

    ax[1].set_xlabel('Connectivity degree\n (b) Total link resource consumption')
    ax[1].set_ylabel('TLRC')
    ax[1].grid(lw=0.25, clip_on=False)

    # if True:
    #     # Creating inset axes for zoom-in detail on specific columns of ILP-Ring and H-Ring
    #     axins = inset_axes(ax[1], width="100%", height="100%",
    #                        # loc='upper left',
    #                        # loc=1,
    #                        loc='upper left',
    #                        bbox_to_anchor=(0.13, 1 - 0.15, .10, .25), bbox_transform=ax[1].transAxes,
    #                        # bbox_to_anchor=(0.05, 0.5, 1, 1),
    #                        # bbox_to_anchor=[50, 80],
    #                        borderpad=1)
    #     inset_indices = [0, 1, 2]  # Indices for ILP-Ring and H-Ring
    #     # inset_indices = [0]
    #     # selected_data = [[av_matrix[2, 2], av_matrix[2, 3]], [av_matrix[3, 2], av_matrix[3, 3]]]
    #     selected_data = [[twc_matrix[1,1]], [twc_matrix[2, 1]], [twc_matrix[3, 1]]]
    #     inset_data = np.array(selected_data)
    #     # inset_data = av_matrix[:, [2, 3]]  # Select the last two columns data for ILP-Ring and H-Ring
    #     inset_ticks = np.array([0])  # Position for the two bars
    #     inset_color_list = [color_list[1], color_list[2], color_cycle[3]]
    #
    #     for i, index in enumerate(inset_indices):
    #         axins.bar(inset_ticks + width * i, inset_data[index], width, alpha=0.7, color=inset_color_list[index],
    #                   hatch=patterns[index])
    #     axins.set_xticks(inset_ticks + width * 0.5)
    #     axins.set_xticklabels(['6'])  # Labels specifically for the zoomed columns
    #     axins.set_ylim(1030, 1070)
    ...
    ax[2].set_xlabel('Connectivity degree\n (c) VLs reconnected with sharing')
    ax[2].set_ylabel('Number of VLs\nreconnected with sharing')
    ...
    if double_axis_flag:
        ...
        arrow_start_x = ellipse_center_x + ellipse_width / 2 - 0.05  # 椭圆左侧边缘
        arrow_start_y = ellipse_center_y
        arrow_end_x = ellipse_center_x + ellipse_width / 2 + 0.48  # 指向y轴的方向
        arrow_end_y = ellipse_center_y
        # 添加紫色虚线箭头
        # -|>
        arrow_style = patches.FancyArrowPatch((arrow_start_x, arrow_start_y), (arrow_end_x, arrow_end_y),
        ...
        arrow_end_x = ellipse_center_x + 0.5# - 0.42 #0.42  # 指向y轴的方向
    ...
    # ######################## Add purple dashed arrow for the second subplot ########################################
    # # 椭圆中心和大小
    # ellipse_center_x = 1.15
    # ellipse_center_y = 1050
    # ellipse_width = 0.8
    # ellipse_height = 150
    #
    # # 在柱状图上添加紫色虚线椭圆
    ...
    # arrow_start_y = ellipse_center_y + ellipse_height / 2
    # arrow_end_x = ellipse_center_x #+ 0.5  # - 0.42 #0.42  # 指向y轴的方向
    # arrow_end_y = ellipse_center_y + ellipse_height / 2 + 1.5 * ellipse_height
    # # 添加紫色虚线箭头
    # # -|>
    # arrow_style = patches.FancyArrowPatch((arrow_start_x, arrow_start_y), (arrow_end_x, arrow_end_y),
    #                                       connectionstyle="arc3", color='purple',
...
def analyze_availability(topology, av_matrix, twc_matrix, shared_link_matrix):
    # connectivity degree
    x_tick_label_list = ['0.5', '0.6', '0.7', '0.8', '0.9', '1']
    scenario_legend_list = ["SP", "SVNM", "SINC", "SINC+"]
    file_name = "description/large_instance_increasing_virtual_links_" + topology + "_av.txt"

    with open(file_name, "w") as file:
        # The availability of SINC can already achieve around xx% when the connectivity degree is 0.5
        file.write("The availability of SINC can already achieve around " + str(av_matrix[2, 0]) + "% when the connectivity degree is 0.5\n")
        file.write("\n")
        # The availability of SINC can be up to around xx\% higher than \textit{SP}
        difference_sinc_sp = av_matrix[2] - av_matrix[0]
        max_difference_sinc_sp = np.max(difference_sinc_sp)
        file.write("The availability of SINC can be up to around " + str(max_difference_sinc_sp) + "% higher than SP\n")
        # The availability of SINC can be up to around xx\% and xx\% higher than \textit{SVNM}
        difference_sinc_svnm = av_matrix[2] - av_matrix[1]
        max_difference_sinc_svnm = np.max(difference_sinc_svnm)
        file.write("The availability of SINC can be up to around " + str(max_difference_sinc_svnm) + "% higher than SVNM\n")

        file.write("\n")
        # this improvement in availability is achieved with up to around xx\% additional required TRLC compared to \textit{SP}
        difference_tw_sinc_sp = twc_matrix[2] - twc_matrix[0]
        max_difference_tw_sinc_sp = np.max(difference_tw_sinc_sp)
        file.write("This improvement in availability is achieved with up to around " + str(max_difference_tw_sinc_sp) + " additional required TRLC compared to SP\n")
        # this improvement in availability is achieved with up to around xx\% additional required TRLC compared to \textit{SVNM}
        difference_tw_sinc_svnm = twc_matrix[2] - twc_matrix[1]
        max_difference_tw_sinc_svnm = np.max(difference_tw_sinc_svnm)
        file.write("This improvement in availability is achieved with up to around " + str(max_difference_tw_sinc_svnm) + " additional required TRLC compared to SVNM\n")

        file.write("\n")
        # SINC+ can always has up to xx% additional availability compared to SINC
        difference_av_sinc_plus_sinc = av_matrix[3] - av_matrix[2]
        max_difference_sinc_lus_sinc = np.max(difference_av_sinc_plus_sinc)
        file.write("SINC+ can always has up to " + str(max_difference_sinc_lus_sinc) + " additional availability compared to SINC\n")
        # SINC+ can always has up to xx% additional TLRC compared to SINC.
        difference_tw_sinc_plus_sinc = twc_matrix[3] - twc_matrix[2]
        max_difference_tw_sinc_plus_sinc = np.max(difference_tw_sinc_plus_sinc)
        file.write("SINC+ can always has up to " + str(max_difference_tw_sinc_plus_sinc) + " additional TLRC compared to SINC\n")
...
if __name__ == '__main__':
    ...
    one_column_matrix_list = get_matrix_columns(topology=cur_topology, num_of_vn=cur_num_of_vn,
                                                num_of_vl_list=cur_num_of_vl_list,
                                                num_instance=cur_num_instance,
                                                scenario_list=scenario_list, scenario_legend_list=scenario_legend_list,
    ...
    total_link_resource_consumption_list = get_matrix_columns(topology=cur_topology, num_of_vn=cur_num_of_vn,
                                                              num_of_vl_list=cur_num_of_vl_list,
                                                              num_instance=cur_num_instance,
                                                              scenario_list=scenario_list, scenario_legend_list=scenario_legend_list,
                                                              column_index_list=[5, 6, 7])
    twc_matrix_survivable = total_link_resource_consumption_list[1]

    # cur_topology = "tokyo-5nodesVN-SP"
    cur_topology = "tokyo-5nodesVN-SP-Capacity"
    ...
    av_matrix_sp, twc_matrix_sp, shared_link_matrix_sp \
        = get_av_twc_shared_link(topology=cur_topology, num_of_vn=cur_num_of_vn,
                                 ...
                                 num_instance=cur_num_instance,
                                 scenario_list=scenario_list, scenario_legend_list=scenario_legend_list)
    # todo: reset the twc of shortest path
    # total link resource consumption: 5, 6, 7
    total_link_resource_consumption_list = get_matrix_columns(topology=cur_topology, num_of_vn=cur_num_of_vn,
                                                                num_of_vl_list=cur_num_of_vl_list,
                                                                num_instance=cur_num_instance,
                                                                scenario_list=scenario_list, scenario_legend_list=scenario_legend_list,
                                                                column_index_list=[5, 6, 7])
    twc_matrix_sp = total_link_resource_consumption_list[1]
    ...
    # round the availability to 2 decimal places
    av_matrix_combine = np.round(av_matrix_combine, 2)
    ...
    analyze_availability(topology="tokyo", av_matrix=av_matrix_combine, twc_matrix=twc_matrix_combine,
                         shared_link_matrix=shared_link_matrix_combine)

    cur_topology = "tokyo-5nodesVN-Capacity"
    # the maximum usage: 8, 9, 10
    maximum_usage_list = get_columns_max_list(topology=cur_topology, num_of_vn=cur_num_of_vn, num_of_vl_list=cur_num_of_vl_list,
                                              num_instance=cur_num_instance,
                                              scenario_list=scenario_list, scenario_legend_list=scenario_legend_list,
