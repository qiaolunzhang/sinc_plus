...
def analyze_availability(topology, av_matrix, twc_matrix, computation_time_matrix, scenario_legend_list):
    ...
    with open(file_name, "w") as file:
        # the availability of SP in H-Ring is xx%
        file.write("The availability of SP in H-Ring is " + str(av_matrix[0, 1]) + "%.\n")
        # the availability of SVNM in ILP-Ring is xx%
        file.write("The availability of SVNM in ILP-Ring is " + str(av_matrix[1, 0]) + "%.\n")
        # the availability of SINC in ILP-Ring is xx%
        file.write("The availability of SINC in ILP-Ring is " + str(av_matrix[2, 0]) + "%.\n")
        file.write("\n")

        # SVNM for ILP-Ring improves the availability by xx% compared to SP in H-Ring
        improvement = av_matrix[1, 0] - av_matrix[0, 1]
        improvement = round(improvement, 2)
        file.write("SVNM for ILP-Ring improves the availability by " + str(improvement) + "% compared to SP in H-Ring.\n")
        ...
        file.write("\n")
        # SVNM for ILP-Ring requires xx% additional wavelength compared to SP in H-Ring
        additional_wavelength = twc_matrix[1, 0] - twc_matrix[0, 1]
        additional_wavelength_percentage = round(additional_wavelength / twc_matrix[0, 1] * 100, 2)
        file.write("SVNM for ILP-Ring requires " + str(additional_wavelength_percentage) + "% additional wavelength compared to SP in H-Ring.\n")
