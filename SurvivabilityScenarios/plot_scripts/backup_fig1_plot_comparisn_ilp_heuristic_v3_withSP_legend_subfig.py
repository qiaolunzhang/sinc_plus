...
def plot_av_twc_computation_time_sp_new(topology, av_matrix, twc_matrix, computation_time_matrix, double_axis_flag=False):
    fig_width = 3.487
    fig_height = fig_width / 1.618
    plot_settings(fig_width, fig_height)  # Assume plot_settings adjusts figure aesthetics

    # Create figure and subplots
    fig, ax = plt.subplots(1, 3, figsize=(fig_width * 3, fig_height))

    # Common setup
    x_tick_label_list = ['ILP-Ring', 'H-Ring', 'ILP-Mesh', 'H-Mesh']
    ind = np.arange(4)  # Original index positions
    width = 1 / (len(av_matrix[0])+1)  # Compute width based on the number of scenarios
    color_cycle = plt.rcParams['axes.prop_cycle'].by_key()['color']
    color_list = [color_cycle[i % len(color_cycle)] for i in range(len(av_matrix))]
    patterns = ('//////', '\\\\\\\\', '---', 'xxx', 'ooo', '\\', '\\\\', '++', '*', 'O', '.')

    label_position = ind + width * (+1)

    # Plotting subplots for Availability, TWC, and Computation Time
    # Availability subplot
    for i in range(len(av_matrix)):
        ax[0].bar(ind + width * i, av_matrix[i], width, alpha=0.7, label=f"Scenario {i+1}", color=color_list[i], hatch=patterns[i])
    ax[0].set_xticks(label_position)
    ax[0].set_xticklabels(x_tick_label_list)
    ax[0].set_xlabel('Scenarios \n (a) Availability')
    ax[0].set_ylabel('Availability')
    ...
    # TWC subplot
    for i in range(len(twc_matrix)):
        ax[1].bar(ind + width * i, twc_matrix[i], width, alpha=0.7, color=color_list[i], hatch=patterns[i])
    ax[1].set_xticks(label_position)
    ax[1].set_xtick_labels(x_tick_label_list)
    ax[1].set_xlabel('Scenarios \n (b) Wavelength consumption')
    ax[1].set_ylabel('TWC')

    # Computation Time subplot with inset for the last two bars of the third scenario
    for i in range(len(computation_time_matrix)):
        ax[2].bar(ind + width * i, computation_time_matrix[i], width, alpha=0.7, color=color_list[i], hatch=patterns[i])
    ax[2].set_xticks(label_position)
    ax[2].set_xtick_labels(x_tick_label_list)
    ax[2].set_xlabel('Scenarios \n (c) Computation time')
    ax[2].set_ylabel('Computation time (s)')
    ax[2].set_yscale('log')

    # Creating inset axes for zoom-in detail
    axins = inset_axes(ax[2], width="30%", height="30%", loc='upper right', borderpad=1.5)
    axins.bar(ind[-2:] + width * 2, computation_time_matrix[2][-2:], width, alpha=0.7, color=color_list[2], hatch=patterns[2])
    axins.set_xticks(ind[-2:] + width * 2)
    axins.set_xticklabels(x_tick_label_list[-2:])
    axins.set_yscale('log')
    axins.set_title('Zoom on Last Two Bars')

    # Adjust layout
    fig.subplots_adjust(wspace=0.4)  # Adjust the spacing between subplots if necessary
    plt.show()
...
def plot_av_twc_computation_time_sp(topology, av_matrix, twc_matrix, computation_time_matrix, double_axis_flag=False):
    ...
    # j is for ZR and long-haul transponders
    for i in range(len(av_matrix)):
        ax[0].bar(ind + width * (i - 1), av_matrix[i], width, alpha=0.7, label=scenario_legend_list[i],
                  color=color_list[i], hatch=patterns[i])
    ...
    axins = inset_axes(ax[0], width="100%", height="100%",
                       ...
                       # loc=1,
                       loc='upper left',
                       bbox_to_anchor=(0.07, 1 - 0.3, .3, .3), bbox_transform=ax[0].transAxes,
    ...
    for i, index in enumerate(inset_indices):
        axins.bar(inset_ticks + width * i, inset_data[index], width, alpha=0.7, color=inset_color_list[index], hatch=patterns[index])
