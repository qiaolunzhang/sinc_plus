...
def different_mapping2(totMaps, link, totCut, totAv, wave, scenario, no_increasing_wave_flag=True):
    # the version to try different local search
    if link == (3, 18, 20):
        print("########### Test ##################")
    ...
    len_previous_link_mapping = len(totMaps[link])

    shortestKPaths = []
    for counter, path in enumerate(X):
        #print(path)
        # left if the number of nodes, right is the nubmer of links
        if len(path) > len_previous_link_mapping + 1 + 3:
            break
        shortestKPaths.append(path)
        if counter == num_shortest_path - 1:
            break

    for sp in shortestKPaths:
        sp_links = []
        for jP in range(len(sp)):
            if jP < len(sp) - 1:
                sp_links.append((sp[jP], sp[jP + 1]))
...
def different_mapping3(totMaps, link, totCut, totAv, wave, scenario, no_increasing_wave_flag=True):
    plink_start_index = 0

    for plink_start_index in range(len(totMaps[link]) - 1):
        P2 = P.copy()
        plink_cur_index = plink_start_index

        # 1-2-3-4-5
        while plink_cur_index < len(totMaps[link]) - 1:
            plink = totMaps[link][plink_cur_index]
            plink_cur_index += 1
            ...
            for vn in range(1, num_vn + 1):
                for link in totMaps:
                    ...
                    if (link[0] == vn):
                        ...
                        totAv = different_mapping3(totMaps, link, totCut, totAv, totW, scenario)
                for link in totMaps:
                    # Consider only virtual links of the VN selected
                    if (link[0] == vn):
                        # print("Trying with vn and link:", vn, link)
                        ...
                        if scenario == 2 or scenario == 4:
                            totAv = different_mapping3(totMaps, link, totCut, totAv, totW, scenario, no_increasing_wave_flag=False)
